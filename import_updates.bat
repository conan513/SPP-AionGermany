...
for %%i in ("%mainfolder%\AL-Game_6.0\sql\update\*sql") do if %%i neq "%mainfolder%\AL-Game_6.0\sql\update\*sql" if %%i neq "%mainfolder%\AL-Game_6.0\sql\update\*sql" if %%i neq "%mainfolder%\AL-Game_6.0\sql\update\*sql" echo %%i & "%mainfolder%\Database\bin\mysql.exe" --defaults-extra-file="%mainfolder%\Database\connection.cnf" --default-character-set=utf8 -f --local-infile %database% --database=%database% < %%i
